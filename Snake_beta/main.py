...
snake_next_boost  = 12
root = tkinter.Tk()
maximum_screen_width = root.winfo_screenwidth()
maximum_screen_hight = root.winfo_screenheight()
...
class Button:
    ...
    def back_home(self):
        global settings, show_history, screen,show_more_highscores1,show_last_highscores1, show_more_highscores,show_last_highscores, show_more, show_less, screen_width, screen_hight
        settings = False
        show_history = False
        screen = pygame.display.set_mode((cell_number_x * cell_size, cell_number_y * cell_size), RESIZABLE)
        screen_width = 800
        screen_hight = 800
        show_more_highscores1 = False
        show_last_highscores1 = False
        show_more_highscores = False
        show_last_highscores = False
    ...
    def check_click_history(self):
        ...
        if self.top_rect.collidepoint(mouse_pos):
            ...
            self.dynamic_elevaition = self.elevaition
            self.top_color = "Blue"
    def show_more_highscores(self):
        global show_more_highscores, show_more_highscores1
        show_more_highscores = True
        show_more_highscores1 = True
    def update_show_more(self):
        self.draw()
        self.check_key_show_more()
    def check_key_show_more(self):
        mouse_pos = pygame.mouse.get_pos()
        if self.top_rect.collidepoint(mouse_pos):
            self.top_color = "Blue"
            if pygame.mouse.get_pressed()[0]:
                ...
                if self.pressed == True:
                    self.dynamic_elevaition = self.elevaition
                    self.pressed = False
                    self.show_more_highscores()


        else:
            self.dynamic_elevaition = self.elevaition
            self.top_color = "Blue"
    def show_last_highscores(self):
        global show_last_highscores, show_last_highscores1
        show_last_highscores = True
        show_last_highscores1 = True
    def update_show_last(self):
        self.draw()
        self.check_key_show_last()
    def check_key_show_last(self):
        mouse_pos = pygame.mouse.get_pos()
        if self.top_rect.collidepoint(mouse_pos):
            self.top_color = "Blue"
            if pygame.mouse.get_pressed()[0]:
...
next_page_button = Button("Next page",screen_width // 3,40,(screen_width // 2 - screen_width // 6,600), 7, None)
last_page_button = Button("Previous page",screen_width // 3,40,(screen_width // 2 - screen_width // 6,600), 7, None)
...
x = highscore_history['Highscores_history']
x = x.get("3highscore")
print(x)
while go:
    ...
    if game_over and not settings:
        ...
        if save_highscore:
            ...
            Vornummer = last_element_of_the_history
        ...
        if show_history:
            ...
            sure_not_too_often = 0
            sure_not_too_often2 = 0


            while True:

                    f = highscore_history['Highscores_history'].get(f"{values_while}highscore")
                    values_while += 1
                    Forenumber += 1

                    if next_page:
                        sure_not_too_often += 1
                        if sure_not_too_often == 1:
                            screen.fill((0, 0, 100))

                            y_coord = 120
                            y_coord1 = 120
                            if values_while <= Vornummer - 10:
                                values_while += 10
                                values_while1 += 10
                                Forenumber += 10
                            else:
                                values_while = Vornummer
                                values_while1 = Vornummer
                                Forenumber = Vornummer
                            continue

                    if last_page:
                        sure_not_too_often2 += 1
                        if sure_not_too_often2 == 1:
                            screen.fill((0, 0, 100))
                    ...
                    y_coord += 40
                    if Forenumber <= amount_of_elements:
                        if show_more_highscores:
                            if amount_of_elements <= Vornummer:
                                amount_of_elements += 500
                                show_more = False
                                show_less = True
                                next_page = True
                                last_page = False
                                ...
                                continue
                            show_more_highscores = False
                        if show_last_highscores:
                            if amount_of_elements > 10:
                                amount_of_elements -= 500
                                last_page = True
                                next_page = False
                                sure_not_too_often = 0
                            ...
                            continue
                        if show_more:
                            next_page_button.update_show_more( )
                        if show_less:
                            last_page_button.update_show_last( )
                        if show_more_highscores1:
                            amount_of_elements1 += 500
                            show_more_highscores1 = False
                        if show_last_highscores1:
                            if amount_of_elements > 10:
                                amount_of_elements1 -= 500
                            show_last_highscores1 = False
                        if amount_of_elements >= Vornummer:
            ...
            while True:
                ...
                g = highscore_history["Time_set"].get(f"{values_while1}Score made in:")
                values_while1 += 1
                if Forenumber1 <= amount_of_elements1:
                    Forenumber1 += 1
                    y_coord1 += 40
                    font_history = pygame.font.SysFont("Calibri", 20, True, False)
